...
- `rememberActionTargets: boolean`: 即时操作 API（如 `aiTap`、`aiInput`）是否按 prompt 记住定位结果。这个开关只控制记忆是否记录；真正提取复用仍需在调用时传 `useMemory: true`。默认值为 true。
- `rememberLocateTargets: boolean`: 定位 API（`aiLocate`、`aiLocateAll`）是否按 prompt 记住定位结果。这个开关只控制记忆是否记录；真正提取复用仍需在调用时传 `useMemory: true`。默认值为 true。
...
const agent = new PuppeteerAgent(page, {
  // Prompt memory 默认会记录，所以这两个配置通常可以省略。
  rememberActionTargets: true,
  rememberLocateTargets: true,
});

const agentWithoutPromptMemory = new PuppeteerAgent(page, {
  rememberActionTargets: false,
  rememberLocateTargets: false,
});
```
...
- 参数：
  ...
  - `options?: Object` - 可选，一个配置对象，包含：
    ...
    - `deepThink?: 'unset' | true | false` - 当模型支持时（取决于 MIDSCENE_MODEL_FAMILY），是否开启规划阶段的深度思考能力。默认值为 `'unset'`（等同于省略该参数），跟随模型服务商的默认策略。当显式设置为 `true` 或 `false` 时，会覆盖环境变量 `MIDSCENE_MODEL_REASONING_ENABLED` 的配置。[详情参阅 deepThink 说明](./model-strategy#关于-aiact-方法的-deepthink-参数)。
...
- 参数：
  ...
  - `options?: Object` - 可选，一个配置对象，包含：
    ...
    - `xpath?: string` - 目标元素的 xpath 路径，用于执行当前操作。如果提供了这个 xpath，Midscene 会优先使用该 xpath 来找到元素，然后依次使用缓存和 AI 模型。默认值为空
    - `cacheable?: boolean` - 当启用 [缓存功能](./caching.mdx) 时，是否允许缓存当前 API 调用结果。默认值为 true
    - `useMemory?: boolean` - 是否在调用 AI 定位前优先尝试 prompt 记忆缓存。`aiLocate` / `aiLocateAll` 默认值为 false。
...
// 手动开启本次定位的记忆复用
await agent.aiLocate('页面顶部的登录按钮', {
  useMemory: true,
});
```
...
对于需要定位元素的 API（`aiTap`、`aiInput`、`aiLocate` 等），Midscene 会基于规范化后的 prompt 生成内部记忆键，并默认先记录下来。

- Prompt memory 默认会记录；只有在你想禁用记录时才需要显式配置。
- 即时操作 API（`aiTap`、`aiInput`、`aiHover` 等）里，`useMemory` 默认是 `false`。
- 定位 API（`aiLocate`、`aiLocateAll`）里，`useMemory` 默认也为 `false`。
- 在 `aiLocate([prompts])` 中，同一次调用里重复的 prompt 会在内部通过索引后缀进行区分。

```typescript
// 全局控制：对这个 agent 实例关闭 prompt memory 记录
const agent = new PuppeteerAgent(page, {
  rememberActionTargets: false,
  rememberLocateTargets: false,
});

// 单次控制：只对当前 locate 手动开启记忆复用
await agent.aiLocate('页面顶部的登录按钮', {
  useMemory: true,
});
```
