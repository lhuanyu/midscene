...
除了通用缓存清理外，Midscene 还提供了基于 prompt 记忆的清理接口：

- `agent.clearMemoryByPrompt(prompt)`：清理由该 prompt 派生的记忆记录。
- `agent.clearAllMemory()`：清理所有由定位相关 API 产生的记忆记录。

示例：

```typescript
await agent.clearMemoryByPrompt('页面顶部的登录按钮');
