...
## Prompt Memory Controls

In addition to general cache cleanup, Midscene also provides prompt-memory cleanup APIs on `agent`:

- `agent.clearMemoryByPrompt(prompt)`: remove memory records derived from a prompt.
- `agent.clearAllMemory()`: remove all memory records created by locate-related APIs.

Example:

```typescript
await agent.clearMemoryByPrompt('The login button at the top of the page');
await agent.clearAllMemory();
```
